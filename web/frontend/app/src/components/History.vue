...
const deleteReadings = async () => {
    ...
    const confirmed = window.confirm(
        `Are you sure you want to delete all history data for sensor ${displayName.value}?\n\nThis action cannot be undone.`
    )
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) {
                throw new Error('No readings found to delete')
            }
            throw new Error(`HTTP error! status: ${response.status}`)
        }
        
        // Clear the current readings data
        readings.value = {
            mac: sensorMac.value,
            timestamps: [],
            humidity: [],
        ...
        // Update charts to show empty state
        updateCharts()
        
        // Show success message
        alert('Sensor history deleted successfully')
        
    } catch (err) {
        error.value = err instanceof Error ? err.message : 'Failed to delete readings'
...
<template>
    ...
    <div class="flex flex-wrap mt-3 grid-gap-lg">
        ...
        <button 
            ...
            :disabled="loading || deleting || !hasData" 
            class="btn btn-danger btn-small btn-with-icon"
            :title="deleting ? 'Deleting...' : 'Delete all history for this sensor'"
        >
            <i :class="deleting ? 'bi bi-hourglass-split' : 'bi bi-trash'"></i>
