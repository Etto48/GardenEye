...
constexpr float BATTERY_SCALING_M = 2.1029388570527314; // mb+q scaling factor for battery voltage divider
constexpr float BATTERY_SCALING_Q = 0.2915682666526469; // mb+q offset for battery voltage divider
...
constexpr uint8_t LED_PIN = 2; // GPIO of internal led

constexpr uint64_t MAX_SENSOR_READINGS = 24; // max number of readings to store in RTC memory
constexpr uint64_t SAMPLING_INTERVAL_S = 20; // sample every N seconds
...
constexpr float CRITICAL_V = 2.0; // critical battery voltage

#define SERVER_HOSTNAME "gardeneye"
